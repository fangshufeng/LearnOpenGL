...
GLFWwindow *createWindow() {
    // createWindow
    GLFWwindow *window = glfwCreateWindow(SCR_W, SCR_H, "混合-半透明", nullptr, nullptr);
    if (window == nullptr) {
...
//    glDepthFunc(GL_LESS);
    ...
    // 创建着色器程序
    Shader shader(FileSystem::getGLSLPath("10-混合/blend_alpha.vs").c_str(), FileSystem::getGLSLPath("10-混合/blend_alpha.fs").c_str());
    
    // 加载纹理
    unsigned int cubeTexture =  loadTexture(FileSystem::getTexturePath("marble.jpg"));
    unsigned int planeTexture =  loadTexture(FileSystem::getTexturePath("metal.png"));
    unsigned int transparentTexture =  loadTexture(FileSystem::getTexturePath("blending_transparent_window.png"));
