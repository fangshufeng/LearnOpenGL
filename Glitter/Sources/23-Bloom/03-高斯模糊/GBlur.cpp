...
GLFWwindow *createWindow() {
    // createWindow
    GLFWwindow *window = glfwCreateWindow(SCR_W, SCR_H, "03-高斯模糊", nullptr, nullptr);
    if (window == nullptr) {
...
bool openBloom = false;
bool openBloomKeyPressed = false;
...
void processInput(GLFWwindow *window) {
    ...
    // 是否开启泛光
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS && !openBloomKeyPressed) {
        openBloom = !openBloom;
        openBloomKeyPressed = true;
    }
    
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_RELEASE) {
        openBloomKeyPressed = false;
    }
}
...
#pragma clang diagnostic pop
    ...
    Shader finalShader(FileSystem::getGLSLPath("23-Bloom/01-场景准备/final.vs").c_str(),
                       FileSystem::getGLSLPath("23-Bloom/03-高斯模糊/final_change.fs").c_str());
    ...
    Shader gBlurShader(FileSystem::getGLSLPath("23-Bloom/03-高斯模糊/g_blur.vs").c_str(),
                       FileSystem::getGLSLPath("23-Bloom/03-高斯模糊/g_blur.fs").c_str());
...
//        glEnable(GL_DEPTH_TEST);
        ...
        for (unsigned int i = 0; i < amount; i++) {
            ...
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            gBlurShader.setInt("horizontal", horizontal);
...
//        renderQuad();
        ...
        finalShader.setBool("openBloom", openBloom);
